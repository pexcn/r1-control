...
function update_qrcode(){
	qrcode.src = 'http://'+control_host+'/pic/nopic.jpg';
	qrcode.qrsig = '';
	localStorage.removeItem('qrcode_state');
	localStorage.removeItem('qrcode_data');
	localStorage.removeItem('qrcode_app');
...
function update_list1(data){
		...
		if(play_index > -1){
			...
			var span = document.createElement('span');
			span.innerHTML = '[正在播放]';
			//span.style.color = 'rgba(238, 0, 0, 1)';
		...
		if(info.PlayerType == 2){
            ...
            if(music_info != null){
				...
				if(ver+1 > 1863 && info.PlayerType == 2){
					...
					var Current_MusicData = info.music_info.Current_MusicData;
					if(typeof(Current_MusicData) == 'object' && typeof(Current_MusicData.api_data) == 'object'){
						if(Current_MusicData.api_data.quality){
							var text = '当前：'+Current_MusicData.api_data.quality;
							if(Current_MusicData.Format){
								
								if(Current_MusicData.api_data.br > 0){
									text += ' '+parseInt(Current_MusicData.api_data.br/1000)+'Kbps';
								}else if(Current_MusicData.Format.bitrate > 0){
									if(Current_MusicData.Format.sampleMimeType != "audio/flac"){
										text += ' '+parseInt(Current_MusicData.Format.bitrate/1000)+'Kbps';
									}
								}
								
								if(Current_MusicData.Format.sampleRate > 0){
									text += ' '+(Current_MusicData.Format.sampleRate/1000)+'kHz';
								}
...
function musicpic_background(){
	...
	if(url != '' && url != 'http://localhost/' && url.indexOf('nopic') == -1){
		...
		img += 'transform: translate(-3%,-3%); filter: blur(60px) brightness(100%); contrast(100%);';//opacity: 0.7;
		set_background_css(img);
		main_div.style = 'position: fixed; z-index: 1; left: 0; top: 0; width: 100%; height: 100%; overflow: auto;background-color: rgba(0,0,0,0.2);';
	}else{
		Restore_background();
	}
}
...
function create_music_info_window(data){
	...
	var div1 = document.createElement('div');
	div1.style = 'margin: 10px 10px 0px 0px; display: flex; justify-content: center; align-items: flex-start;flex-direction: column;';
	div.appendChild(div1);
	title_div.appendChild(div);
	var div = div1;
